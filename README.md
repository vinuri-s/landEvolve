...
Every field in *Input Setup*, the process picker, and each process's own parameters has a small **ⓘ** info icon — hover it for a plain-language explanation of what the field does, without leaving the setup screen.

1. **Browse for an input DEM** (`Browse...`) in *Input Setup*. Once selected, the satellite *Location Preview* centres on the DEM and a labelled details line beneath the map shows its metadata — size, **resolution (m)** (shown as `W×H m` if the pixels aren't square), CRS, and elevation range. The boundary toggle overlays the DEM's extent on the map. Before a DEM is selected (or if one fails to preview), the map shows a friendly placeholder card instead of a blank panel.
...
4. **Select earth surface processes you want to simulate**: click **+ Add Process**, pick a type from the list (each shown with a plain-language name and description) — e.g. *Water Flow Routing*, *Erosion & Sediment Transport*, *Hillslope Soil Creep*, *Rainfall & Runoff*, *Vegetation Cover*, *Rock Layers*, *Tectonic Uplift* — then fill in its parameters in the dialog that opens. Added processes appear in the table with their own **Edit**/**Remove**; the picker only offers types not already added. *Water Flow Routing* is flagged with a **Required** badge, since the two Erosion processes need it to run; *Rainfall & Runoff* additionally needs it to actually affect erosion.
5. **Choose an output folder** — its own section at the end of setup, separate from the simulation parameters above — pre-filled with the app's default outputs location (or whichever folder you picked last time), overridable via `Browse...`. Each run writes to a `simulation_<N>/` subfolder here, numbered independently per output folder.
...
5. Open the **Feature Tracking Map** tab in the results. It's an interactive, scrubbable heatmap animation cropped to just the tracked polygon (plus a little surrounding context), with the **first-change** (cyan circle) and **biggest-change** (gold star) events marked directly on the map — each labelled with when it happened, the magnitude, and its location. If a first effect was detected, the animation opens on the frame nearest that time instead of the blank starting frame.
    *   The full time series behind that map is also written to the run's output folder: `feature_tracking.csv` (columns: time, mean/max/min elevation, mean change, **mean geomorphic change** (uplift removed), max absolute change, volume change) and `feature_tracking.png` — a two-panel elevation / erosion-deposition plot over time, for further analysis outside the app. On **Tectonics** runs, the erosion/deposition panel plots the uplift-removed geomorphic change (with the raw total change, uplift included, as a thin reference line) — a feature can be net *rising* from uplift while still eroding underneath it, and the panel is about the latter.
...
*   **`SimulationWindow`**: The main setup screen for configuring simulation parameters, selecting earth surface processes, browsing for an input DEM file, and choosing an output folder (both from the user's own filesystem). Every field has a hoverable **ⓘ** info icon explaining what it's for. A satellite **Location Preview** map (centred on the selected DEM) can overlay its boundary, shows a friendly placeholder before a DEM is chosen, and a labelled details line beneath the map shows the DEM's metadata — size, **resolution (m)** (both dimensions if pixels aren't square), CRS, and elevation range.
...
*   **`ComponentController`**: Backs the process picker/add dialogs — lists available components, resolves each one's plain-language display name and prerequisite badge (read from the `Component` row itself; see Data Layer below), and builds the config-dialog field layout from a component's parameters.
...
*   **`Components`** (`app/engine/components.py`): each is a thin wrapper exposing a Landlab process to the app, or custom logic layered on top.
    ...
    *   `VegetationComponent`: applies per-class multipliers to erodibility (`K_sed`, `K_br`), hillslope diffusivity, and runoff. Supports **Static** (one class throughout) and **Transition** (scheduled class changes at given **simulated years**, tracked against elapsed time rather than a step count — so a transition fires at the same simulated year regardless of what Time Step the run uses) modes.
...
*   **Ordering guarantees.** Runoff is set before flow routing; vegetation/lithology K is applied before SPACE reads it; `LithoLayers` refreshes `K_sp` after erosion so the next step sees the newly exposed rock. `FlowAccumulator` (*Water Flow Routing*) is required for `SpaceComponent`/`SpaceLargeScaleEroderComponent` (*Erosion & Sediment Transport* / *Erosion (Large-Scale)*) to receive discharge — flagged with a "Required" badge in the process picker — but is **not** needed by `DepthDependentDiffuserComponent` (*Hillslope Soil Creep*), which computes its own slope field independently.
...
*   **`Database`** (instance `db_manager`): Handles SQLite connection and session management. `create_tables()` creates any missing tables, then calls `_sync_schema()` — a lightweight, additive-only migration that adds any column present in the SQLAlchemy models but missing from an already-existing table (SQLite's `create_all()` only creates whole tables, never alters one that's already there). This matters beyond a fresh dev checkout: a packaged build's `app_data.db` sits beside the executable and survives an app upgrade, so a model gaining a new column would otherwise crash that install with "no such column" instead of just picking it up.
*   **Models**: `Component`/`ComponentParam`, `Lithology`, and `VegetationClass`. (Input DEMs are **not** stored in the database — the user browses for a GeoTIFF on their own filesystem at run time.) Both `Component` and `ComponentParam` carry presentation metadata — `Component` has `display_name` and `prerequisite_badge`/`prerequisite_tooltip` (e.g. *Water Flow Routing* → "Required"); `ComponentParam` has `display_name`, `units`, `description` — so the picker and configuration dialogs show layman-friendly names without the technical Landlab class/parameter names ever reaching the UI. `seed.py`'s `backfill_component_metadata()` fills these in for any row that predates the columns (i.e. seeded by an older app version), touching only `display_name IS NULL` rows so it never overwrites a user's own edits; called once at startup alongside `seed_database()`.
...
*   **Coordinate system handling (metres, built at the origin).** The simulation physics (slopes, drainage area, sediment volumes, SPACE) works in **metres**, and the compute grid is always built at the origin `(0, 0)` regardless of the DEM's real-world location:
    ...
    *   **Non-square pixels are supported.** The grid's `dx`/`dy` are read independently from the source raster (not collapsed to one value), so a DEM with non-square pixel spacing still gets the correct cell area, slope, and drainage-area calculations — and the DEM-info panel reports both dimensions (`W×H m`) whenever they differ.
