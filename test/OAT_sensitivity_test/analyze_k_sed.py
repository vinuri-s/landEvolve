...
def run_sensitivity_analysis(min_k_sed, max_k_sed, steps, k_br, duration, output_file_name, dem_path):
    """
    Runs OAT sensitivity analysis for K_sed.
    """
    k_sed_values = np.logspace(np.log10(min_k_sed), np.log10(max_k_sed), steps)
    results = []
    
    script_dir = os.path.dirname(os.path.abspath(__file__))
    current_test_dir = get_next_test_dir_with_prefix(script_dir, "k_sed", "ksed_sensitivity_test")
    
    # Full path for CSV output
    final_output_csv = os.path.join(current_test_dir, os.path.basename(output_file_name))
        
    print(f"Starting Sensitivity Analysis for K_sed...")
    print(f"Test Directory: {current_test_dir}")
    print(f"DEM: {dem_path}")
    print(f"Range: [{min_k_sed}, {max_k_sed}]")
    print(f"Constant K_br: {k_br}")
    print(f"Steps: {steps}")
    print(f"Duration: {duration} years per run")
    print("-" * 40)

    for i, k_sed in enumerate(k_sed_values):
        # Coupled relationship: K_br is 1/100th of K_sed (bedrock harder than sediment)
        k_br = k_sed * 0.01
        print(f"Run {i+1}/{steps}: K_sed = {k_sed:.2e}, K_br = {k_br:.2e}")
        ...
        run_folder_name = f"run_{i+1}_ksed_{k_sed:.2e}_{int(duration)}yr"
        ...
        save_output_png(os.path.join(run_dir, f"{base_name}_elevation.png"), 
                        final_elevation, shape, f"Final Elevation (K_sed={k_sed:.2e})", cmap='terrain')
        
        save_output_png(os.path.join(run_dir, f"{base_name}_difference.png"), 
                        difference, shape, f"Topographic Difference (K_sed={k_sed:.2e})", cmap='seismic_r')
