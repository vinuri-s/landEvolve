from PyQt6.QtWidgets import (
    ...
    QTextEdit, QGridLayout, QGroupBox, QScrollArea, QSizePolicy
)
...
class SimulationStatsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.setMinimumSize(500, 400)  # Keep it resizable
        ...
        # --- Performance Metrics (Scrollable) ---
        ...
        layout.addWidget(perf_scroll, 1)  # Give it stretch
        ...
        layout.addWidget(details_scroll, 2)  # Give it more stretch than performance metrics
...
# --- The rest of SimulationResultsWindow remains the same ---
# Your full SimulationResultsWindow class from your previous code can stay unchanged
...
class SimulationResultsWindow(QMainWindow):
    ...
    def update_progress(self, percentage, status_message):
        ...
        # Count simulation steps for statistics
        if "Step" in status_message:
            self.simulation_steps += 1
    
    def update_live_stats(self):
        """Update live time and RAM display"""
        # Update elapsed time
    ...
    def show_stats_dialog(self):
        """Show simulation statistics dialog"""
    ...
    def get_grid_size_info(self):
        """Get grid size information from simulation parameters"""
        try:
            # Extract grid dimensions from simulation parameters
    ...
    def get_detailed_info(self):
        """Generate detailed simulation information"""
        try:
            details = []
            ...
            # Add simulation parameters safely
            for key, value in self.sim_params.items():
                if key not in ['selected_components']:
                    try:
                        details.append(f"{key}: {value}")
                    except:
                        details.append(f"{key}: [Unprintable value]")
            
            # Add components info safely
    ...
    def _get_component_name(self, comp):
        """Extract component name safely from either dict or object"""
        try:
            ...
            # Try object attributes
                ...
                component_obj = comp.component
                return getattr(component_obj, 'name', 
                            getattr(component_obj, '__class__').__name__)
            
        except Exception as e:
            print(f"Debug - component type: {type(comp)}")
            print(f"Debug - component attributes: {dir(comp) if hasattr(comp, '__dict__') else 'No dict'}")
